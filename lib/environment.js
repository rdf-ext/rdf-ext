function mixin (rdf) {
  ...
  rdf.PrefixMap.prototype.isCURIEWithDefinedPrefix = function (val) {
    var separator = val.indexOf(':')
    var prefix = (separator !== -1) ? val.substr(0, separator).toLowerCase() : undefined
    return (prefix !== undefined && prefix in this)
  }
  ...
  rdf.RDFEnvironment = function () {
    ...
    return Object.defineProperties(new rdf.Profile(rp), {
      ...
      createNamedNode: {
        writable: false, configurable: false, enumerable: true, value: function (curieOrIri) {
          if (this.prefixes.isCURIEWithDefinedPrefix(curieOrIri)) {
            return new rdf.NamedNode(this.prefixes.resolve(curieOrIri))
          } else {
