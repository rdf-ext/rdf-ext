...
var LdpStore = function (rdf, options) {
  ...
  self.add = function (iri, graph, callback) {
    ...
    self.serializers[mimeType](graph, function (data) {
      self.request('POST', iri, {'Content-Type': mimeType}, data,
        function (statusCode, headers, content, error) {
  ...
  self.update = function (iri, graph, callback) {
    var mimeType = 'text/turtle';
    
    self.serializers[mimeType](graph, function (data) {
      self.request('PUT', iri, {'Content-Type': mimeType, 'If-Match': graph.etag}, data,
        function (statusCode, headers, content, error) {
          callback(graph, error);
        }
      );
  ...
  self.merge = function (iri, graph, callback) {
    ...
    self.serializers[mimeType](graph, function (data) {
      self.request('PATCH', iri, {'Content-Type': mimeType, 'If-Match': graph.etag}, 
        'INSERT DATA { ' + data + ' }',
        function (statusCode, headers, content, error) {
          callback(graph, error);
        }
